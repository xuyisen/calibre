...
class Build(Command):
    ...
    def build_headless(self):
        ...
        if os.path.exists(bdir):
            shutil.rmtree(bdir)
        os.makedirs(bdir)
        # Copy and rewrite CMakeLists.txt based on Qt version
        src_cmake = os.path.join(os.path.dirname(sources[0]), 'CMakeLists.txt')
        dst_cmake = os.path.join(bdir, 'CMakeLists.txt')
        with open(src_cmake) as f:
            cmake_content = f.read()
        qt_version = qt.get('version', (6, 0))
        if qt_version >= (6, 10):
