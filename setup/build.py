...
class Build(Command):
    ...
    def build_headless(self):
        ...
        if os.path.exists(bdir):
            shutil.rmtree(bdir)
        # Copy headless source into the build tree so we can modify CMakeLists.txt
        src_dir = os.path.dirname(sources[0])
        work_dir = self.j(bdir, 'src')
        shutil.copytree(src_dir, work_dir)
        # Rewrite CMakeLists.txt based on Qt version
        cmake_file = self.j(work_dir, 'CMakeLists.txt')
        with open(cmake_file) as f:
            cmake_content = f.read()
        qt_ver = qt.get('version', (6, 0))
        if qt_ver >= (6, 10):
            ...
            link_targets = 'target_link_libraries(headless PRIVATE Qt::Gui Qt::Core)'
        ...
        os.chdir(bdir)
        try:
            self.check_call(cmd + ['-S', work_dir])
