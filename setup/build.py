...
class Build(Command):
    ...
    def build_headless(self):
        ...
        if os.path.exists(bdir):
            shutil.rmtree(bdir)
        os.makedirs(bdir)
        # Copy headless sources into the build tree
        src_dir = os.path.dirname(sources[0])
        for f in os.listdir(src_dir):
            sf = os.path.join(src_dir, f)
            if os.path.isfile(sf):
                shutil.copy2(sf, os.path.join(bdir, f))
        # Rewrite CMakeLists.txt based on Qt version
        cmake_path = os.path.join(bdir, 'CMakeLists.txt')
        with open(cmake_path) as f:
            cmake_content = f.read()
        qt_major, qt_minor = qt['version']
        qt_prefix = f'Qt{qt_major}'
        if qt_major >= 6 and qt_minor >= 10:
            find_gui = f'find_package({qt_prefix} REQUIRED COMPONENTS Gui GuiPrivate Core CorePrivate)'
            link_targets = f'target_link_libraries(headless PRIVATE {qt_prefix}::Gui {qt_prefix}::GuiPrivate {qt_prefix}::Core {qt_prefix}::CorePrivate)'
        else:
            find_gui = f'find_package({qt_prefix} REQUIRED COMPONENTS Gui Core)'
            link_targets = f'target_link_libraries(headless PRIVATE {qt_prefix}::Gui {qt_prefix}::Core)'
